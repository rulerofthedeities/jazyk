...
export abstract class ReadnListenSentencesComponent implements OnInit, OnDestroy {
  ...
  private processChapter(chapterData: ChapterData, audioChapterData: AudioChapterData, position: Position) {
    if (chapterData && chapterData.chapter) {
      ...
      // console.error(activeSentences.length === activeAudioSentences.length ? 'length equal' : 'Error: LENGTH NOT EQUAL!!');
      ...
      if (activeSentences.length === activeAudioSentences.length) {
        ...
        activeSentences.forEach((sentence, i) => {
          if (sentence.text !== activeAudioSentences[i].text) {
            // console.error('Error: TEXT different', i, `>${sentence.text}<`, `>${activeAudioSentences[i].text}<`);
            // Prevent incorrect audio / sentence match
            sentencesMatch = false;
          }
        });
      }
      // console.error(sentencesMatch ? 'sentences match' : '!sentences don\'t match');
